# User Service

## Описание проекта

Консольное приложение `user-service`, разработанное в рамках курса "Интенсив Java".

Цель проекта — реализовать CRUD-операции для сущности User с использованием Hibernate и PostgreSQL без применения Spring Framework.

Технологии:
- Java
- Maven
- Hibernate ORM
- PostgreSQL
- JDBC
- Log4j / SLF4J

Требования:

- Использовать Hibernate в качестве ORM.
- База данных — PostgreSQL.
- Настроить Hibernate без Spring, используя hibernate.cfg.xml или properties-файл.
- Реализовать CRUD-операции для сущности User (создание, чтение, обновление, удаление), которая состоит из полей: id, name, email, age, created_at.
- Использовать консольный интерфейс для взаимодействия с пользователем.
- Использовать Maven для управления зависимостями.
- Настроить логирование.
- Настроить транзакционность для операций с базой данных.
- Использовать DAO-паттерн для отделения логики работы с БД.
- Обработать возможные исключения, связанные с Hibernate и PostgreSQL

## Предметная область

Проект представляет собой сервис управления пользователями.

Сущность User описывает зарегистрированного пользователя системы и содержит информацию:
- имя;
- email;
- возраст;
- дату создания записи.

Сервис поддерживает базовые операции управления пользователями (CRUD).

## Структура проекта
```
src/
├── main/
│    ├── java/
│    │   └── ru/
│    │       └── aston/
│    │           └── user/
│    │               └── service/
│    │                   ├── Main.java
│    │                   ├── config/
│    │                   │   └── HibernateUtil.java
│    │                   ├── dao/
│    │                   │   └── UserDao.java
│    │                   │   └── UserDaoImpl.java
│    │                   ├── entity/
│    │                   │   └── User.java   
│    │                   ├── exception/
│    │                   │   ├── EmailCheckException.java
│    │                   │   ├── UserNotDeletedException.java
│    │                   │   ├── UserNotFoundException.java
│    │                   │   ├── UserNotSaveException.java
│    │                   │   ├── UserNotUpdateException.java
│    │                   │   └── ValidationException.java
│    │                   ├── service/
│    │                   │   └── UserService.java
│    │                   └── ui/
│    │                       └── ConsoleUI.java
│    │
│    └── resources/
│        ├── hibernate.cfg.xml
│        └── logback.xml
├── pom.xml
├── README.md
└── .gitignore
```
## Команда

| Участник        | Роль                 |
|-----------------|----------------------|
| Валерия Фурсова | Team Lead, Developer |
| Вадим Сымоник   | Developer            |
| Александр Франк | Developer            |

## Распределение задач

| Участник        | Задачи                                                                                                                                                                |
|-----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Валерия Фурсова | - настройка Maven-проекта<br/>- создание Entity User<br/>- настройка Hibernate<br/>- исправить NibemateUtil<br/>- составить тесты для основных методов                |
| Вадим Сымоник   | - реализация DAO<br/>реализация CRUD-операций<br/>- консольный интерфейс<br/>- исправить ошибки в методах update и DAO<br/>- составить набор тестов для сохранения    |
| Александр Франк | - настройка логирования<br/>- обработка исключений<br/>- тестирование<br/>- исправить ошибки с исключениями<br/>- составить набор тестов для обновления               |

## Сложности и вопросы

Будут добавляться по мере выполнения проекта.
1) Как правильно писать тесты на методы для проверки взаимодействия с БД? Проблема, что при любом тесте создается новая сущность и дублируется, а удалить ее после тестов не получается.
 Из-за этого некоторые тесты могут работать некоректно.
2) Как используются библиотека Mockito и для чего нужны ее аннотации?
3) Как в целом должен проходить процесс тестирования?
